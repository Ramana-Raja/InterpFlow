...
class VideoImport(ctk.CTkFrame):
    def __init__(self, parent, import_func):
        ...
        self.select_video_btn = ctk.CTkButton(button_frame, text='Select Video', command=self.select_video, width=150, height=40, corner_radius=10)
        ...
        self.batch_var = ctk.StringVar(value='12')
        self.width_var = ctk.StringVar(value='680')
        ...
        ctk.CTkLabel(input_frame, text="Width:").grid(row=0, column=3, padx=(5, 0))
        self.width_entry = ctk.CTkEntry(input_frame, textvariable=self.width_var, width=60)
        self.width_entry.grid(row=0, column=4, padx=(0, 10))

        ctk.CTkLabel(input_frame, text="Height:").grid(row=0, column=5, padx=(5, 0))
        self.height_entry = ctk.CTkEntry(input_frame, textvariable=self.height_var, width=60)
        self.height_entry.grid(row=0, column=6, padx=(0, 10))

        ctk.CTkLabel(input_frame, text="Model:").grid(row=0, column=7, padx=(5, 0))
        self.height_entry = ctk.CTkEntry(input_frame, textvariable=self.model_var, width=60)
        self.height_entry.grid(row=0, column=8, padx=(0, 10))

        self.trt_checkbox = ctk.CTkCheckBox(input_frame, text="TRT", variable=self.trt_var)
        self.trt_checkbox.grid(row=0, column=9, padx=(10, 0))
    ...
    def select_video(self):
        ...
        if video_file:
            ...
            self.select_video_btn.configure(text="Video Selected")
